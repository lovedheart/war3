 ...
 * Data layer facade.
 *
 * ⚠ INTERIM SHIM — the data-tables agent owns `schema.ts` / `load.ts` and the
 * JSON under `data/`. This file defines the SHAPE the sim codes against and,
 * until those tables land, falls back to a compact built-in set so the game is
 * runnable end-to-end. When the real loader lands it must keep these exports:
 *   getGameData(): GameData, validateAll(raw), type UnitDef/BuildingDef/…
 */
...
function buildFallback(): GameData {
